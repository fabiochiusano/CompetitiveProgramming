...
char T[MAX_N], P[MAX_N]; // the input string, up to 100K characters
...
void countingSort(int k) { // O(n)
	int i, sum, maxi = max(300, n); // up to ASCII chars or length of n
...
PII stringMatching() { // string matching in O(m log n)
	...
	if (strncmp(T + SA[lo], P, m) != 0) return PII(-1, -1); // if not found
...
int main() {
	n = (int)strlen(gets(T)); // normal input T, without the '$'
